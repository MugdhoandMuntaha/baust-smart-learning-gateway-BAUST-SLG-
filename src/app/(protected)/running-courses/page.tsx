...
export default function RunningCoursesPage() {
  ...
  const theoryCourses = courses.filter((c) => c.type === "theory" || !c.type);
  const sessionalCourses = courses.filter((c) => c.type === "sessional");

  const renderCourseCard = (course: Course) => (
    <Grid size={{ xs: 12, sm: 6, md: 4 }} key={course.id}>
      <Link
        href={`/documents?folder=${encodeURIComponent(course.name)}`}
      ...
      {loading ? (
        ...
        <Box className="space-y-10">
          ...
          {theoryCourses.length > 0 && (
            <Box>
              <Typography variant="h6" sx={{ fontWeight: 700, color: "#006B3F", mb: 2, borderBottom: "2px solid #E2E8F0", pb: 1 }}>
          ...
          {/* Section 2: Sessional Courses */}
          {sessionalCourses.length > 0 && (
            <Box>
              <Typography variant="h6" sx={{ fontWeight: 700, color: "#1B4F72", mb: 2, borderBottom: "2px solid #E2E8F0", pb: 1 }}>
                Sessional Courses ({sessionalCourses.length})
              </Typography>
              <Grid container spacing={3}>
                {sessionalCourses.map(renderCourseCard)}
              </Grid>
            </Box>
          )}
        </Box>
